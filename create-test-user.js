...
async function createTestUser() {
  ...
  const goals = [
    {
      ...
      due_date: new Date(Date.now() + 7 * 7 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // 7 weeks from now
      order_index: 0,
    },
    {
      ...
      due_date: new Date(Date.now() + 7 * 7 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
      order_index: 1,
    },
    {
  ...
  // 5. Create completion data for weeks 1-5 (simulating week 5 progress)
  const currentWeek = getWeekNumber(new Date());
  const startWeek = currentWeek.week - 4; // 4 weeks ago (we're in week 5)
  ...
  console.log('✓ Created completion data for 5 weeks');

  // 6. Create Weekly Plans for weeks 1-5
  const weeklyPlans = [
    {
      week: 1,
      plans: [
  ...
  console.log('✓ Created weekly plans for 5 weeks');

  // 7. Create WAM responses for weeks 1-4 (not current week yet)
  const wamResponses = [
    {
      week: 1,
  ...
  console.log('✓ Created WAM responses for 4 weeks');
  ...
  console.log('Current Week: 5 of 12');
  console.log('Average Score: ~78%');
  console.log('\nLogin at: https://blue-suite-irg3qknqr-rprovines-projects.vercel.app');
}
